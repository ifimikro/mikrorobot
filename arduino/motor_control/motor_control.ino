...
int speed_accelerate = 10; // How much to increment speed with.
int speed_standard = 20; // Standard starting speed when changing from L/R/F/B.
...
// Start going FORWARD at pre-defined speed.
    ...
    } else if (cmd_msg.data == 2) { //LEFT
        if (prev_state == 2) {
// Increase motor speed.
            motor_incr_speed();

        } else if (prev_state == 1 || prev_state == 4) { // Turn left, forwards or backwards.
            if (motor1_speed <= (255-speed_accelerate)) {
                motor1_speed += speed_accelerate;
                motor1.setSpeed(motor1_speed);
            }
...
// Start going LEFT at pre-defined speed.
    ...
    } else if (cmd_msg.data == 3) { //RIGHT
        if (prev_state == 3) {
// Increase motor speed.
            motor_incr_speed();
            
        } else if(prev_state == 1 || prev_state == 4) { // Turn right, forwards or backwards.
            if (motor2_speed <= (255-speed_accelerate)) {
                motor2_speed += speed_accelerate;
                motor2.setSpeed(motor2_speed);
            }
